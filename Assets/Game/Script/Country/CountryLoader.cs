...
public class CountryLoader : MonoBehaviour
{
	...
	public TextAsset centerFile;
	public List<string> excludedCountryCodes;
	...
	public void Load()
	{
		if (!loaded || !Application.isPlaying)
		{
			if (countryFile != null)
			{
				...
				countries = countryReader.ReadCountries(countryFile , centerFile, excludedCountryCodes , countryFlags);
			}
	...
	public bool IsExcludedCountry(int index)
	{
		for (int i = 0; i < countries.Length; i++)
            {
                if (index == countries[i].countryIndex)
                {
					return false;
                }
